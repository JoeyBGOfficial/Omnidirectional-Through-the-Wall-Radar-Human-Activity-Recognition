...
if Config.Evaluation.Run_Full_Testing
    disp('Evaluating cross-orientation testing set...');
    ...
    disp('Cross-orientation testing summary:');
    disp(Orientation_Results.Orientation_Table);
    if Orientation_Results.Pass_Target
        fprintf('All testing orientations satisfy the %.2f%% target.\n', ...
            Config.Evaluation.Target_Minimum_Orientation_Accuracy);
    else
        fprintf('Minimum testing accuracy is %.2f%%. Please tune only by validation-set feedback.\n', ...
...
disp('One-key training workflow has finished.');
